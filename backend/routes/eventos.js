  ...
  try {
    ...
    const schedResult = await conn.execute(
      `SELECT COUNT(*) AS TOTAL,
         ...
         FROM HORARIO_BIBLIOTECA        WHERE COD_BIBLIOTECA = :cod_bib`,
    ...
    const result = await conn.execute(
      `INSERT INTO EVENTO         (ID_EVENTO, TITULO_EVENTO, DESCRICAO_EVENTO, LOCAL_EVENTO, DATA_EVENTO,
...
router.post('/:id/avaliacoes', autenticar, async (req, res) => {
  ...
  try {
    ...
    const presResult = await conn.execute(
      `SELECT PRESENCA_CONFIRMACAO FROM PARTICIPACAO_EVENTO       WHERE ID_EVENTO = :id AND NUM_CARTAO = :nc`,
      { id: parseInt(req.params.id), nc: num_cartao },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
  ...
  try {
    ...
    const result = await conn.execute(
      `SELECT ID_HORARIO_EV, DIA_SEMANA,
         ...
         FROM HORARIO_EVENTO        WHERE ID_EVENTO = :id
...
router.patch('/:id/participantes/:num_cartao/presenca', autenticar, async (req, res) => {
  ...
  try {
    conn = await getConnection();
    const upd = await conn.execute(
      `UPDATE PARTICIPACAO_EVENTO          SET PRESENCA_CONFIRMACAO = :presenca
